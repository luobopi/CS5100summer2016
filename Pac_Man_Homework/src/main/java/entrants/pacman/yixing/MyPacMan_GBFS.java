...
public class MyPacMan_GBFS extends PacmanController {
    ...
    public MOVE getMove(Game game, long timeDue) {
        ...
        /// Strategy 2: Find nearest edible ghost and go after them used Greedy Best-First Search
        ...
        for (GHOST ghost : GHOST.values()) {
            ...
            if (game.getGhostEdibleTime(ghost) > 0) {
                ...
                for(int i = 0; i < neighbours.length; i++) {
                    double distance = game.getEuclideanDistance(neighbours[i], game.getGhostCurrentNodeIndex(ghost));
...
//                        minGhost = ghost;
        ...
        if(minNeighbour != -1) {
            return game.getNextMoveTowardsTarget(current, minNeighbour, DM.PATH);
        }
